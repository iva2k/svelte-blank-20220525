...
const config: PlaywrightTestConfig = {
  ...
  webServer: {
    command: 'pnpm run svelte:build:only && pnpm run preview', // TODO: having "pnpm run svelte:build" calls "cap sync android", which hits a bug with no extension windows command file crashing in node --experimental-loader
    port: 3000,
    timeout: 120 * 1000
  }
